...
# Force UTF-8 encoding for stdout to prevent crashes on Windows console
...
def chatbotResponse(msg, source_language):
    print(f"Input ({source_language}): {msg}")
    
    # Translate to English for processing
    translated_msg = translate_message(msg, source_language, 'en')
    print(f"Translated to EN: {translated_msg}")
    
    # Predict and Get Response
    ints = predict_class(translated_msg, model)
    res = getResponse(ints, intents)
    print(f"EN Response: {res}")
    
    # Translate back to source language
    translated_response = translate_message(res, 'en', source_language)
    print(f"Final Response ({source_language}): {translated_response}")
